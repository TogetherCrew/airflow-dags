...
def fetch_repo_review_comments_page(owner: str, repo: str, page: int, per_page: int = 100):
    """
    ...
    updated_response_data = list(map(lambda x: {**x, "pull_request_number": extract_pull_request_number_from_review_comment_response(x)}, response_data))
...
def fetch_repo_issues_and_prs_comments_page(owner: str, repo: str, page: int, per_page: int = 100):
    """
    Fetches the review comments for all pull requests in a GitHub repository.
    ...
    updated_response_data = list(map(lambda x: {**x, **extract_type_from_comment_response(x)}, response_data))
