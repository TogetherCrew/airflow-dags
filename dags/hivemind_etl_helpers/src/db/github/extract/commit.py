...
class GithubCommitExtraction:
    ...
    def _fetch_raw_commits(
        ...
        with self._neo4j_connection.connect_neo4j().session() as session:
            raw_records = session.execute_read(self._exec_query, repoIds=repository_id, from_date=from_date)
        return raw_records
