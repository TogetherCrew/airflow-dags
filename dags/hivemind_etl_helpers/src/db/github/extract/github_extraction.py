...
# 1. The Extraction class seems to be more about orchestrating these different data-fetching operations rather than being a specialized version of them, which suggests a "has-a" relationship more than an "is-a" relationship.
# 2. Avoidance of Multiple Inheritance
# 3. Flexibillity of the Extraction class
# 4. Encapsulation and Modularity

# it's essential to evaluate whether the relationship between your classes truly fits the "is-a" model. In your case, composition not only avoids the complexities associated with multiple inheritance but also promotes better design principles by keeping your classes focused, loosely coupled, and easier to maintain.
...
class GithubExtraction:
    def __init__(self):
        ...
        # to be uncommented once other pull requests regarding `extraction` merges
    ...
    def fetch_comments(self, repository_id: list[int], from_date: datetime | None = None, **kwargs) -> list[GitHubComment]:
        return self.comment_extraction.fetch_comments(repository_id, from_date, **kwargs)

# With dependency injection

# class GithubExtraction:
#     def __init__(self, comment_extraction, issues_extraction, pull_requests_extraction):
...
#         pull_requests_data = self.pull_requests_extraction.fetch_pull_requests()
