...
class TestSaveCommitRelationToPRCheckDuplicateCommit(TestCase):
    """
    We need to make sure we won't create duplicate nodes
    """
    ...
    def test_pre_saved_commit(self):
        pr = [
            {
        ...
        self.neo4j_driver.execute_query("CREATE (c:Commit) SET c.sha = 'random_sha'")
        save_commits_relation_to_pr(
            commit_sha="random_sha", repository_id="1234", pull_requests=pr
        )

        data_commits = self.neo4j_driver.execute_query("MATCH (c:Commit) RETURN c")
