...
with DAG(
    dag_id="telegram_summary_vector_store",
    start_date=datetime(2024, 1, 1),
    schedule_interval="0 3 * * *",
    catchup=False,
    max_active_runs=1,
    ...
    def processor(
        ...
        ingestion_pipeline = CustomIngestionPipeline(
            community_id=community_id, collection_name="telegram_summary"
        )
        ...
        documents = transformer.transform(summaries=summaries)
        logging.info(f"{len(summaries)} Messages transformed!")
        ingestion_pipeline.run_pipeline(docs=documents)
        logging.info("Finished loading into database!")
